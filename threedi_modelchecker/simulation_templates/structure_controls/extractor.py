...
class StructureControlExtractor(object):
    ...
    def __initialize_controls(self):
        if self._controls is None:
            ...
            table_lookup = dict([(x.id, x) for x in Query(ControlTable).with_session(self.session).all()])
            memory_lookup = dict([(x.id, x) for x in Query(ControlMemory).with_session(self.session).all()])
            timed_lookup = dict([(x.id, x) for x in Query(ControlTimed).with_session(self.session).all()])
            maps_lookup = {}
            ...
            all_controls = Query(
                [Control, ControlGroup, ControlMeasureGroup]).join(
                    ControlGroup, ControlMeasureGroup).with_session(self.session).filter(Control.control_group_id==self._control_group_id, ControlGroup.id==self._control_group_id).all()
