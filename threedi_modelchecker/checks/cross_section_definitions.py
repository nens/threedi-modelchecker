...
class CrossSectionVariableRangeCheck(CrossSectionBaseCheck):
    def __init__(
        ...
        **kwargs,
        ...
        if min_value is None:
            ...
            self.range_str += f"{'>=' if left_inclusive else '>'} {min_value}"
        if max_value is None:
            ...
            self.range_str += f"{'<=' if right_inclusive else '<'} {max_value}"
    ...
    def description(self):
        return f"{self.column_name} should be in range {self.range_str}"
