...
from .threedi_model.models import constants, Base, DECLARED_MODELS
...
class Config:
    ...
    def __init__(self, models: Base = DECLARED_MODELS):
        self.models = models
        self._checks = []
        self.generate_checks()
        self._checks = Checks(self.checks)
    ...
    def generate_checks(self):
        ...
        self._checks += FOREIGN_KEY_CHECKS
        self._checks += UNIQUE_CHECKS
        self._checks += NOT_NULL_CHECKS
        self._checks += INVALID_TYPE_CHECKS
        self._checks += INVALID_GEOMETRY_CHECKS
        self._checks += INVALID_GEOMETRY_TYPE_CHECKS
        self._checks += INVALID_ENUM_CHECKS
        self._checks += OTHER_CHECKS
        self._checks += TIMESERIES_CHECKS
        self._checks += RANGE_CHECKS
        self._checks += CONDITIONAL_CHECKS
        return None
...
class Checks:
    ...
    def __getattr__(self, item):
        if self.table is None:
            mapping_tables = dict()
            for check in self.checks:
                _checks_table = mapping_tables.setdefault(check.table.name, [])
                _checks_table.append(check)
            return Checks(mapping_tables.get(item, []), table=item)
        else:
            mapping_column = dict()
            for check in self.checks:
                _checks_columns = mapping_column.setdefault(check.column.name, [])
                _checks_columns.append(check)
            return Checks(
                mapping_column.get(item, []), table=self.table, column=item
            )
    ...
    def __repr__(self):
        if not self.table:
            return f"<Checks ALL: {len(self)}>"
        elif not self.column:
            return f"<Checks {self.table}: {len(self)}>"
        else:
            return f"<Checks {self.table}.{self.column}: {len(self)}>"
