...
UPDATE v2_global_settings SET minimum_sim_time_step = min(0.01, sim_time_step) WHERE minimum_sim_time_step IS NULL OR minimum_time_step <= 0 OR minimum_time_step > sim_time_step;
UPDATE v2_global_settings SET maximum_sim_time_step = sim_time_step WHERE (maximum_sim_time_step IS NULL OR maximum_sim_time_step < sim_time_step) AND timestep_plus = 1;
UPDATE v2_global_settings SET output_time_step = max(1, sim_time_step) WHERE output_time_step IS NULL OR output_time_step < sim_time_step;
...
DELETE FROM v2_aggregation_settings SET aggregation_method = 'med';
UPDATE v2_aggregation_settings SET flow_variable = 'discharge' WHERE flow_variable = 'discharge_negative' AND aggregation_method = 'cum_negative';
UPDATE v2_aggregation_settings SET flow_variable = 'discharge' WHERE flow_variable = 'discharge_positive' AND aggregation_method = 'cum_positive';
"""

def upgrade():
    op.execute(MIGRATION_QUERIES)
